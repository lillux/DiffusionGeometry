...
class BaseTensorSpace:
    ...
    def metric(self) -> "np.ndarray":
        ...
        return metric_gram.metric(u_coeffs, self.cdc_components)

    def metric_apply(
        self, a_coeffs: "np.ndarray", b_coeffs: "np.ndarray"
    ) -> "np.ndarray":
        """Apply the metric to two coefficient vectors."""
        from ...src.metric_gram import _metric_apply
    ...
    def gram(self) -> "np.ndarray":
        ...
        """
        u_coeffs = self.dg.function_basis[:, : self.dg.n_coefficients]
        return metric_gram.gram(u_coeffs, self.cdc_components, self.dg.triple.measure)
